...
float humidityCalibration = 2.0;  ///!!! CALIBRATION: Add +2.0% to match RuuviTag
...
void setup() {
    ...
    Serial.println("=== Cricket_Peripheral_1 - BLE Environmental Sensor ===");
    ...
    // Rev 2 boards need delay for I2C GPIO enable pins to stabilize
    delay(100);

    if (!HS300x.begin()) {
        delay(500);  // Retry with longer delay
...
void takeSensorReading() {
    ...
    if (sensorInitialized && !sensorError) {
        // rev. 1, calibration with ruuviTag
        temperature = (HS300x.readTemperature()) + (tempCalibration);  // Celsius
...
void takeSensorReading() {
    ...
    if (sensorInitialized && !sensorError) {
        // rev. 1, calibration with ruuviTag
        temperature = HS300x.readTemperature() + tempCalibration;  // Celsius
        humidity = HS300x.readHumidity() + humidityCalibration;  ///!!! CALIBRATION: Apply +2.0% offset
    ...
    // Round average temperature to tenths
    float avgTemp = round(calculateAverage(tempReadings, sampleCount) * 10.0) / 10.0;
    float avgHum  = calculateAverage(humReadings, sampleCount);

    // Format strings with 1 decimal place
    String tempStr    = String(validReading ? temperature : 0.0, 1);
    String humStr     = String(validReading ? humidity : 0.0, 1);
    String avgTempStr = String(avgTemp, 1);
    String avgHumStr  = String(avgHum, 1);
